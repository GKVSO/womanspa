...
const cards = [
  {
    title: "Feel More Comfortable",
    text: "Support feminine\nwellness and daily comfort",
  },
  {
    title: "Restore Confidence",
    text: "Many clients report\nfeeling more connected and confident",
  },
  {
    title: "Non-Surgical Solution",
    text: "No invasive procedures\nor extended downtime",
  },
  {
    title: "Prioritize Yourself Again",
    text: "A wellness-focused approach\ndesigned around self-care and quality of life",
  },
  {
    title: "Luxury Private Care",
    text: "An elevated environment\ndesigned around discretion and comfort",
  },
  {
    title: "Quick, Convenient Sessions",
    text: "Treatments are designed to fit into \nbusy lifestyles with minimal interruption",
  },
];
...
export default function CandelaSmootherBenefits({ cms, editing, hideDevice = false }: { cms?: Record<string, unknown>; editing?: boolean; hideDevice?: boolean }) {
  const t = useT();
    ...
    >
      <motion.div ref={titleRef} style={{ opacity: titleOpacity, y: titleY }} className="will-change-transform">
        <h2 {...e("text","benefits_title")} className={`text-center text-white text-[32px] min-[768px]:text-[48px] min-[1200px]:text-[70px] min-[1600px]:text-[92px] leading-tight font-berlingske font-normal px-2 sm:px-0 ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}>
          {cmsTitle ? t(cmsTitle) : <>{t("More Than A Treatment.")}
          <br />
          {t("A Confidence &amp; Wellness Upgrade")}</>}
        </h2>
      </motion.div>

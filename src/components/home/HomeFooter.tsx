...
export default function HomeFooter({ showOnMobile = true, hideAt1200 = false, showOnlyDesktop = false }: { showOnMobile?: boolean; hideAt1200?: boolean; showOnlyDesktop?: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ formType: "home_footer_consultation", name, phone, concern }),
      });
      if (res.ok) {
        setStatus("success");
        setName("");

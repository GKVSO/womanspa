...
const cards = [
  {
    icon: "/sylfirm-group.svg",
    title: "Redness correction",
    subtitle: "Helps reduce visible redness",
  },
  {
    icon: "/sylfirm-waist.svg",
    title: "Skin refinement",
    subtitle: "Healthier-looking skin quality",
  },
  {
    icon: "/sylfirm-clock.svg",
    title: "Minimal Downtime",
    subtitle: "Back to life in 24 hours",
  },
];

export default function SylfirmHero({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
  ...
  return (
    <section
      {...e("section","hero")}
      className={`bg-cover bg-center bg-no-repeat rounded-b-[60px] px-5 sm:px-10 pt-20 sm:pt-24 pb-10 sm:pb-16 min-h-[100vh] flex flex-col overflow-hidden ${!cmsBg ? "bg-[url(/sylfirm-hero-360.png)] min-[768px]:bg-[url(/sylfirm-hero-768.png)] min-[1200px]:bg-[url(/sylfirm-hero.webp)]" : ""} ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}
      style={cmsBg ? { backgroundImage: `url(${cmsBg})` } : { ["--bg-base" as any]: "url('/sylfirm-hero-360.png')", ["--bg-768" as any]: "url('/sylfirm-hero-768.png')", ["--bg-1200" as any]: "url('/sylfirm-hero.webp')" } }
    >

...
const areas = [
  {
    label: "Face & Neck",
    text: "acne, rosacea, melasma, fine lines, pigmentation, sebaceous hyperplasia",
    side: "left",
  ...
  {
    label: "Any Body Area",
    text: "laser hair removal for all skin types, including fine/vellus hair",
    side: "left",
  ...
  {
    label: "Legs & Arms",
    text: "spider veins, ingrown hairs, keratoses",
    side: "left",
    position: "left-[20%] top-[55%]",
    lineWidth: "330px",
  },
  {
    label: "Back, Shoulders & Chest",
    text: "truncal acne, folliculitis, scarring",
    side: "right",
  ...
  {
    label: "Hands",
    text: "sun damage, age spots, crepiness",
    side: "right",
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <p className="text-[#313242] font-medium text-[14px] leading-snug mt-1">{t(area.text)}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
...
export default function AerolaseTreatmentAreas() {
  const t = useT();
    ...
    <section className="relative overflow-hidden rounded-b-[60px] bg-white px-5 sm:px-10 pt-14 sm:pt-24 pb-20 sm:pb-32">
      <FadeIn as="div" y={30}>
        <h2 className="text-center text-[#313242] text-[32px] sm:text-[48px] leading-tight font-berlingske font-normal mb-6">
          {t("A Complete Skin Health Platform")}
        </h2>
        <p className="text-center text-[#313242] font-medium text-[16px] mb-16">
          {t("Aerolase Neo treats the full body:")}
        </p>
      </FadeIn>

      <div className="relative max-w-[900px] mx-auto h-[700px]">
        <Image
          src="/aerolase-areas-model.webp"
          alt={t("Aerolase Neo treatment areas")}
          width={500}
          height={900}

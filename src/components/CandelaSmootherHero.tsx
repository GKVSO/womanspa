...
const cards = [
  {
    icon: "/emfemme-group.svg",
    title: "Intimate comfort",
    subtitle: "Supports tissue comfort",
  },
  {
    icon: "/emfemme-lotus.svg",
    title: "Feminine wellness",
    subtitle: "Daily comfort support",
  },
  {
    icon: "/emfemme-clock.svg",
    title: "Zero Downtime",
    subtitle: "Back to life immediately",
  },
];

export default function CandelaSmootherHero({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
  ...
  return (
    <section
      {...e("section","hero")}
      className={`bg-cover bg-center bg-no-repeat rounded-b-[60px] px-5 sm:px-10 pt-20 sm:pt-24 pb-10 sm:pb-16 min-h-[100vh] flex flex-col overflow-hidden ${!cmsBg ? "bg-[url(/candela-smoother-hero.webp)]" : ""} ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}
      style={cmsBg ? { backgroundImage: `url(${cmsBg})` } : { ["--bg-base" as any]: "url('/candela-smoother-hero.webp')" } }
    >
      <StaggerContainer staggerDelay={0.15} className="w-full flex-1 flex flex-col">
        ...
        <StaggerItem>
          <p {...e("text","hero_subtitle")} className={`${subtitleFont || ""} text-white text-[16px] font-semibold leading-relaxed mt-6 max-w-[680px] ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`} style={{ ...(cms ? { color: sv(cms.subtitleColor) || undefined, fontSize: cms.subtitleSize ? `${cms.subtitleSize}px` : undefined } : {}) }}>
            {cmsSubtitle ? t(cmsSubtitle) : t("EMFEMME 360 is a non-surgical feminine wellness treatment designed to support comfort, confidence, intimate wellness, and quality of life in a luxury clinical environment")}
          </p>
        </StaggerItem>

...
const cards = [
  {
    icon: "/icon-collagen.svg",
    title: "Collagen stimulation",
    subtitle: "Supports collagen renewal",
  },
  {
    icon: "/icon-tightening.svg",
    title: "Skin tightening",
    subtitle: "Improves skin firmness",
  },
  {
    icon: "/icon-zero-downtime.svg",
    title: "Zero Downtime",
    subtitle: "Back to life immediately",
  },
];

export default function ExionHero({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
  ...
  return (
    <section
      {...e("section","hero")}
      className={`bg-cover bg-center bg-no-repeat rounded-b-[60px] px-5 sm:px-10 pt-20 sm:pt-24 pb-10 sm:pb-16 min-h-[100vh] flex flex-col overflow-hidden ${!cmsBg ? "bg-[url(/exion-hero-360.png)] min-[768px]:bg-[url(/exion-hero-768.png)] min-[1200px]:bg-[url(/exion-hero-1200.png)] min-[1600px]:bg-[url(/exion-hero.webp)]" : ""} ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}
      style={cmsBg ? { backgroundImage: `url(${cmsBg})` } : { ["--bg-base" as any]: "url('/exion-hero-360.png')", ["--bg-768" as any]: "url('/exion-hero-768.png')", ["--bg-1200" as any]: "url('/exion-hero-1200.png')", ["--bg-1600" as any]: "url('/exion-hero.webp')" } }
    >
      <StaggerContainer staggerDelay={0.15} className="w-full flex-1 flex flex-col">
        ...
        <StaggerItem>
          <h1 {...e("text","hero_title")} className={`${titleFont || "font-berlingske"} text-white text-[24px] min-[768px]:text-[36px] min-[1200px]:text-[40px] min-[1600px]:text-[48px] leading-tight whitespace-pre-line ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`} style={{ ...(cms ? { color: sv(cms.titleColor) || undefined, fontSize: cms.titleSize ? `${cms.titleSize}px` : undefined } : {}) }}>
            {cmsTitle ? t(cmsTitle) : <>{t("EXION — For Firmer, Smoother\n& Younger-Looking Skin")}</>}
          </h1>
        </StaggerItem>
        ...
        <StaggerItem>
          <p {...e("text","hero_subtitle")} className={`${subtitleFont || ""} text-white text-[16px] font-semibold leading-relaxed mt-6 max-w-[620px] ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`} style={{ ...(cms ? { color: sv(cms.subtitleColor) || undefined, fontSize: cms.subtitleSize ? `${cms.subtitleSize}px` : undefined } : {}) }}>
            {cmsSubtitle ? t(cmsSubtitle) : t("Non-invasive treatment designed to improve skin firmness, collagen production, texture, and overall skin quality — without surgery or downtime")}
          </p>
        </StaggerItem>
        ...
        <StaggerItem className="mt-auto sm:mt-0">
          <div {...e("button","hero_buttons")} className={`flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mt-8 sm:mt-10 ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`}>
            ...
            <motion.button onClick={() => smoothScrollToTarget('before-after')} {...btnHover} className="w-full sm:w-auto text-white font-bold text-[14px] border border-white rounded-[10px] px-8 py-4 bg-transparent hover:bg-white/10 transition-colors cursor-pointer whitespace-nowrap">

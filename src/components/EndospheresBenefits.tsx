...
const cards = [
  {
    title: "Cellulite Smoothing",
    text: "Supports smoother-looking skin texture and tissue refinement",
  },
  {
    title: "More Athletic Shape",
    text: "Helps support fluid movement and reduce puffiness",
  },
  {
    title: "Heavy Legs & Swelling",
    text: "Popular among clients struggling with heaviness and water retention",
  },
  {
    title: "Body Tone & Firmness",
    text: "Supports firmer-feeling skin and improved contour appearance",
  },
  {
    title: "Circulation Support",
    text: "Promotes healthy blood circulation and tissue oxygenation",
  },
  {
    title: "Recovery & Muscle Relaxation",
    text: "Mechanical vibration may help with recovery support and muscle relief",
  },
];
...
export default function EndospheresBenefits({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
    ...
    >
      <motion.div ref={titleRef} style={{ opacity: titleOpacity, y: titleY }} className="will-change-transform">
        <h2 {...e("text","benefits_title")} className={`text-center text-white text-[32px] min-[768px]:text-[48px] min-[1200px]:text-[70px] min-[1600px]:text-[92px] leading-tight font-berlingske font-normal ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}>
          {cmsTitle ? t(cmsTitle) : <>{t("More Than")}
          <br />
          {t("Cellulite Reduction")}</>}
        </h2>
      </motion.div>
      ...
      >
        <Image
          src={cmsImage || "/endo-benefits-handpiece.webp"}
          alt={t("Endospheres handpiece")}
          width={600}
          height={900}
      ...
      <div ref={cardsRef} className="relative z-20 -mt-8 min-[768px]:-mt-[100px] w-full">
        <div
          className="absolute -inset-15 blur-[30px] bg-[#CBA07D] rounded-[60px]"
        />

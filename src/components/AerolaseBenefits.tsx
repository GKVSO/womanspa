...
const cards = [
  {
    title: "Rapid Acne Clearance",
    text: "The laser penetrates to the sebaceous glands, killing acnes bacteria and reducing inflammation",
  },
  {
    title: "Fades Pigmentation & Melasma",
    text: "Melanin-rich lesions (sunspots, age spots, melasma) are gently fragmented without damaging surrounding skin",
  },
  {
    title: "Calms Rosacea & Redness",
    text: "Diffuse facial redness, telangiectasias, and flushing are visibly reduced as the laser energy selectively coagulates unwanted vessels",
  },
  {
    title: "Stimulates Collagen & Elastin",
    text: "Gentle dermal heating triggers neocollagenesis — softening fine lines, improving texture, and restoring firmness over time",
  },
  {
    title: "Supports Laser Hair Removal",
    text: "Aerolase Neo can also be used for gentle, effective hair reduction on all skin types, including fine vellus hair that other lasers miss",
  },
  {
    title: "All Skin Tones",
    text: "The 1064nm wavelength and microsecond speed make Aerolase Neo one of the safest lasers for darker skin types",
  },
];
...
export default function AerolaseBenefits({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
    ...
    >
      <motion.div ref={titleRef} style={{ opacity: titleOpacity, y: titleY }} className="will-change-transform">
        <h2 {...e("text","benefits_title")} className={`text-center text-white text-[32px] min-[768px]:text-[48px] min-[1200px]:text-[70px] min-[1600px]:text-[92px] leading-tight font-berlingske font-normal px-2 sm:px-0 ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}>
          {cmsTitle ? t(cmsTitle) : <>{t("One Device. 650 Microseconds.")}
          <br />
          {t("All Skin Types. Infinite Possibilities")}</>}
        </h2>
      </motion.div>
      ...
      >
        <Image
          src={cmsImage || "/aerolase-benefits.webp"}
          alt={t("Aerolase Neo technology")}
          width={600}
          height={900}

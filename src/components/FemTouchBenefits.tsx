...
import { ml, toLang } from "@/lib/i18n-helpers";

const cards = [
  {
    title: "Restored Natural Lubrication",
    text: "Many women experience significant\nimprovement in moisture after just 1–2 sessions.\nComfort returns to intimacy — and daily life",
  },
  {
    title: "Improved Vaginal Tone & Elasticity",
    text: "Collagen regeneration gradually restores\ntightness and resilience, enhancing sensation\nfor both partners.",
  },
  {
    title: "Reduction In Stress Urinary",
    text: "FemTouch strengthens the supportive tissue\naround the urethra and bladder neck, reducing\nleaks during physical activity",
  },
  {
    title: "Increased Comfort & Confidence",
    text: "No more planning your day around bathrooms,\navoiding intimacy, or feeling disconnected\nfrom your own body",
  },
  {
    title: "Non-Surgical & Non-Hormonal",
    text: "An ideal option for women who cannot\nor prefer not to use hormone therapy",
  },
];
...
export default function FemTouchBenefits({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
    ...
    >
      <motion.div ref={titleRef} style={{ opacity: titleOpacity, y: titleY }} className="will-change-transform">
        <h2 {...e("text","benefits_title")} className={`text-center text-white text-[32px] min-[768px]:text-[48px] min-[1200px]:text-[70px] min-[1600px]:text-[92px] leading-tight font-berlingske font-normal px-2 sm:px-0 ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}>
          {cmsTitle ? t(cmsTitle) : <>{t("FemTouch: A Few")}
          <br />
          {t("Gentle Minutes. Lasting Change")}</>}
        </h2>
      </motion.div>

      <div ref={cardsRef} className="relative z-20 mt-6 w-full">
        <div

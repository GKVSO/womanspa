...
export default function AerolaseHero({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
  ...
  return (
    <section
      {...e("section","hero")}
      className={`bg-cover bg-center bg-no-repeat rounded-b-[60px] px-5 sm:px-10 pt-20 sm:pt-24 pb-10 sm:pb-16 min-h-[100vh] flex flex-col overflow-hidden ${!cmsBg ? "bg-[url(/aerolase-hero-360.png)] min-[768px]:bg-[url(/aerolase-hero-768.png)] min-[1200px]:bg-[url(/aerolase-hero.webp)]" : ""} ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}
      style={cmsBg ? { backgroundImage: `url(${cmsBg})` } : { ["--bg-base" as any]: "url('/aerolase-hero-360.png')", ["--bg-768" as any]: "url('/aerolase-hero-768.png')", ["--bg-1200" as any]: "url('/aerolase-hero.webp')" } }
    >
      <StaggerContainer staggerDelay={0.15} className="w-full flex-1 flex flex-col">
        ...
        <StaggerItem>
          <h1 {...e("text","hero_title")} className={`${titleFont || "font-berlingske"} text-white text-[24px] min-[768px]:text-[36px] min-[1200px]:text-[40px] min-[1600px]:text-[48px] leading-tight ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`} style={{ ...(cms ? { color: sv(cms.titleColor) || undefined, fontSize: cms.titleSize ? `${cms.titleSize}px` : undefined } : {}) }}>
            {cmsTitle ? t(cmsTitle) : <>{t("Aerolase Neo - Laser For Acne, Rejuvenation")}<br />{t("&amp; Even Tone At")}<br />{t("WO/MAN Luxe MedSpa")}</>}
          </h1>
        </StaggerItem>
        ...
        <StaggerItem>
          <p {...e("text","hero_subtitle")} className={`${subtitleFont || ""} text-white text-[16px] font-semibold leading-relaxed mt-6 max-w-[680px] ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`} style={{ ...(cms ? { color: sv(cms.subtitleColor) || undefined, fontSize: cms.subtitleSize ? `${cms.subtitleSize}px` : undefined } : {}) }}>
            {cmsSubtitle ? t(cmsSubtitle) : t("A revolutionary, single-platform solution that treats acne, clears pigmentation, reduces redness, stimulates collagen, and removes hair — all with virtually no pain and zero downtime")}
          </p>
        </StaggerItem>

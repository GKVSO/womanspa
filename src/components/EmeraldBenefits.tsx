...
const cards = [
  {
    title: "Zero Heat, Zero Pain",
    text: "Targets fat cells without heat, burning, or surgery during treatments",
  },
  {
    title: "No Downtime",
    text: "Clients can return to their day - no compression garments, no needles, no restrictions",
  },
  {
    title: "Clinically Supported Body Contouring",
    text: "Studies report visible circumference reduction, with visible results after the first sessions",
  },
  {
    title: "Lymphatic-Friendly",
    text: "Cold laser supports gentle fat cell release while promoting lymphatic activity and wellness",
  },
];
...
export default function EmeraldBenefits({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
  ...
  return (
    ...
    >
      ...
      >
        <Image
          src={cmsImage || "/emerald-benefits-laser.webp"}
          alt={t("Emerald Laser technology")}
          width={800}
          height={500}
      ...
      <div ref={cardsRef} className="relative z-20 -mt-8 min-[768px]:-mt-[100px] w-full">
        ...
        <div className="relative rounded-[40px]" style={{ backgroundColor: "#CBA07D" }}>
          <div {...e("card","benefits_items")} className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5 sm:gap-8 ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}>
            {cards.map((card, i) => (

...
export default function FemTouchHero({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
  ...
  return (
    <section
      {...e("section","hero")}
      className={`bg-cover bg-center bg-no-repeat rounded-b-[60px] px-5 sm:px-10 pt-20 sm:pt-24 pb-10 sm:pb-16 min-h-[100vh] flex flex-col overflow-hidden ${!cmsBg ? "bg-[url(/femtouch-hero-360.png)] min-[768px]:bg-[url(/femtouch-hero-768.png)] min-[1200px]:bg-[url(/femtouch-hero.webp)]" : ""} ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}
      style={cmsBg ? { backgroundImage: `url(${cmsBg})` } : { ["--bg-base" as any]: "url('/femtouch-hero-360.png')", ["--bg-768" as any]: "url('/femtouch-hero-768.png')", ["--bg-1200" as any]: "url('/femtouch-hero.webp')" } }
    >

...
export default function HydrafacialHero({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  const t = useT();
  ...
  return (
    <section
      {...e("section","hero")}
      className={`bg-cover bg-center bg-no-repeat rounded-b-[60px] px-5 sm:px-10 pt-20 sm:pt-24 pb-10 sm:pb-16 min-h-[100vh] flex flex-col overflow-hidden ${!cmsBg ? "bg-[url(/hydrafacial-hero-360.png)] min-[768px]:bg-[url(/hydrafacial-hero-768.png)] min-[1200px]:bg-[url(/hydrafacial-hero.webp)]" : ""} ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}
      style={cmsBg ? { backgroundImage: `url(${cmsBg})` } : { ["--bg-base" as any]: "url('/hydrafacial-hero-360.png')", ["--bg-768" as any]: "url('/hydrafacial-hero-768.png')", ["--bg-1200" as any]: "url('/hydrafacial-hero.webp')" } }
    >
      <StaggerContainer staggerDelay={0.15} className="w-full flex-1 flex flex-col">
        <StaggerItem>
          <div className="group flex flex-col items-start gap-2 mb-4 sm:mb-6 sm:flex-row sm:items-center sm:flex-wrap">
            ...
            <span className="text-white text-[13px] sm:text-[16px] ml-2">{t("Rated 5.0 on Google &middot; Hallandale Beach, FL")}</span>
          </div>
        </StaggerItem>
        ...
        <StaggerItem>
          <h1 {...e("text","hero_title")} className={`${titleFont || "font-berlingske"} text-white text-[24px] min-[768px]:text-[36px] min-[1200px]:text-[40px] min-[1600px]:text-[48px] leading-tight ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`} style={{ ...(cms ? { color: sv(cms.titleColor) || undefined, fontSize: cms.titleSize ? `${cms.titleSize}px` : undefined } : {}) }}>
            {cmsTitle ? t(cmsTitle) : <>{t("HydraFacial Syndeo - The Most")}<br />{t("Advanced HydraFacial Platform")}<br />{t("at Hallandale Beach")}</>}
          </h1>
        </StaggerItem>
        ...
        <StaggerItem>
          <p {...e("text","hero_subtitle")} className={`${subtitleFont || ""} text-white text-[16px] font-semibold leading-relaxed mt-6 max-w-[680px] ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`} style={{ ...(cms ? { color: sv(cms.subtitleColor) || undefined, fontSize: cms.subtitleSize ? `${cms.subtitleSize}px` : undefined } : {}) }}>
            {cmsSubtitle ? t(cmsSubtitle) : t("Technology delivers deep cleansing, gentle extraction, and intensive hydration in a single 30-minute treatment. Customizable with medical-grade boosters for your unique skin goals. The facial that resets your skin - and your confidence")}
          </p>
        </StaggerItem>
        ...
        <StaggerItem className="mt-auto sm:mt-0">
          <div {...e("button","hero_buttons")} className={`flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mt-8 sm:mt-10 ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`}>
            <BookButton className="w-full sm:w-auto bg-white text-black font-bold text-[14px] rounded-[10px] px-8 py-4 cursor-pointer" label={cmsPrimaryBtn ? t(cmsPrimaryBtn) : t("Book Your HydraFacial")} />
          </div>
        </StaggerItem>
      </StaggerContainer>
    </section>

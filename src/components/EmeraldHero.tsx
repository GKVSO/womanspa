...
export default function EmeraldHero({ cms, editing }: { cms?: Record<string, unknown>; editing?: boolean }) {
  ...
  return (
    <section
      {...e("section","hero")}
      className={`bg-cover bg-center bg-no-repeat rounded-b-[60px] px-5 sm:px-10 pt-20 sm:pt-24 pb-10 sm:pb-16 min-h-[100vh] flex flex-col overflow-hidden ${!cmsBg ? "bg-[url(/emerald-hero-360.png)] min-[768px]:bg-[url(/emerald-hero-768.png)] min-[1200px]:bg-[url(/emerald-hero.webp)]" : ""} ${editing ? "cursor-pointer hover:ring-2 hover:ring-white" : ""}`}
      style={cmsBg ? { backgroundImage: `url(${cmsBg})` } : { ["--bg-base" as any]: "url('/emerald-hero-360.png')", ["--bg-768" as any]: "url('/emerald-hero-768.png')", ["--bg-1200" as any]: "url('/emerald-hero.webp')" } }
    >
      <StaggerContainer staggerDelay={0.15} className="w-full flex-1 flex flex-col">
        ...
        <StaggerItem>
          <p {...e("text","hero_subtitle")} className={`${subtitleFont || ""} text-white text-[16px] font-semibold leading-relaxed mt-6 max-w-[680px] ${editing ? "cursor-pointer hover:ring-2 hover:ring-[#CBA07D]" : ""}`} style={{ ...(cms ? { color: sv(cms.subtitleColor) || undefined, fontSize: cms.subtitleSize ? `${cms.subtitleSize}px` : undefined } : {}) }}>
            {cmsSubtitle ? t(cmsSubtitle) : t("The first and only low-level laser therapy FDA-cleared for individuals with a BMI up to 40. Clinicallyproven to emulsify fat cells without heat, pain, or downtime — now available in Hallandale Beach.")}
          </p>
        </StaggerItem>

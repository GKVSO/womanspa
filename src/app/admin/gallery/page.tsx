...
export default function AdminGallery() {
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: "var(--admin-bg)" }}>
      {/* Top bar */}
      <AdminHeader 
        title={tr(lang, "globalGallery")} 
        extraButtons={
          <>
            <button onClick={addItem} className="px-4 py-1.5 rounded-[8px] bg-gray-200 text-black text-sm font-bold">+ {tr(lang, "add")}</button>
        ...
        {gallery.map((r, i) => (
          <div key={r.id || i} className="p-5 rounded-[14px]" style={{ background: "var(--admin-panel)", border: "1px solid var(--admin-border)" }}>
            <div className="flex justify-between mb-4">
              ...
              <button onClick={() => removeItem(i)} className="text-red-500 text-sm font-bold">{tr(lang, "deleteItem")}</button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div>
                <label className="block text-[12px] mb-1 font-bold text-gray-400">{tr(lang, "beforeImage")}</label>
              ...
              <div>
                <label className="block text-[12px] mb-1 font-bold text-gray-400">{tr(lang, "beforeLabel")} ({lang.toUpperCase()})</label>
                <input type="text" value={ml(r.beforeLabel, lang)} onChange={e => updateItem(i, "beforeLabel", mlSet(r.beforeLabel, lang, e.target.value))} className="w-full px-3 py-2 rounded-[8px]" style={inputStyle} />
              </div>
              <div>
                <label className="block text-[12px] mb-1 font-bold text-gray-400">{tr(lang, "afterImage")}</label>
              ...
              <div>
                <label className="block text-[12px] mb-1 font-bold text-gray-400">{tr(lang, "afterLabel")} ({lang.toUpperCase()})</label>
                <input type="text" value={ml(r.afterLabel, lang)} onChange={e => updateItem(i, "afterLabel", mlSet(r.afterLabel, lang, e.target.value))} className="w-full px-3 py-2 rounded-[8px]" style={inputStyle} />
              </div>
            </div>
          </div>
        ))}

...
- **JSX Syntax for t() hook**: Never use `{t("Text\\nNext")}` directly inside JSX inside conditional rendering, as JS interprets `{}` as an object. Always wrap in fragments: `<>{t("Text\\nNext")}</>`.
...
- **BookButton**: Always add `whitespace-nowrap` to the `<BookButton>` component (e.g. `className="whitespace-nowrap"`) so the text never wraps into 2 lines. Do not add `mt-*` classes directly to the button if it breaks flex layout; wrap the button in a div instead.
